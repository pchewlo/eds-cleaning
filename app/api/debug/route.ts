...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (reset) {
      ...
      await sql`CREATE TABLE candidates (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), job_id text NOT NULL REFERENCES jobs(id) ON DELETE CASCADE, name text, email text, phone text, cv_blob_url text NOT NULL, cv_text text, file_hash text NOT NULL, metadata_json jsonb, rank_score numeric, rank_reasoning text, rank_flags jsonb, uploaded_by uuid REFERENCES users(id), uploaded_at timestamptz DEFAULT now(), ranked_at timestamptz)`;
      await sql`CREATE UNIQUE INDEX candidates_job_id_file_hash_idx ON candidates(job_id, file_hash)`;
      await sql`CREATE INDEX candidates_job_id_rank_score_idx ON candidates(job_id, rank_score DESC)`;
    ...
    const cols = await sql`SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'jobs' AND column_name = 'id'`;
    const rows = await sql`SELECT id, title FROM jobs LIMIT 5`;
    await sql.end();

    return Response.json({
      idType: cols[0]?.data_type,
      jobs: rows,
      resetApplied: reset,
    });
  } catch (e) {
